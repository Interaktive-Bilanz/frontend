...
const SortableAccountItem: React.FC<SortableAccountItemProps> = ({
  ...
  const removeAccountHandler = () => {
    confirmToast(
      `Konto ${account.id} ${account.label} wirklich aus der Position entfernen?`,
      () => onRemove()
    )
  }
      ...
      >
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-y-1">
          <div className="flex flex-row items-center min-w-0 flex-1 w-full">
            ...
            <div lang="de" className="min-w-0 hyphens-auto flex-1">
              {accountId} {account.label}
            </div>
            {teacherMode && (
              <button
                className="bg-gray-100 hover:bg-gray-200 w-8 h-8 px-1 py-1 rounded"
                onClick={(e) => {
                  e.stopPropagation();
